...
class FabricLikeApiModDownloadTask(private val fileName: String, private val versionItem: VersionItem) : InstallTask, Tools.DownloaderFeedback {
    ...
    override fun run(customName: String): File {
        com.craftstudio.launcher.task.TaskExecutors.runInUIThread { ProgressKeeper.submitProgress(ProgressLayout.INSTALL_RESOURCE, 0, R.string.mod_download_progress, versionItem.fileName) }
    ...
    override fun updateProgress(curr: Long, max: Long) {
        ...
        com.craftstudio.launcher.task.TaskExecutors.runInUIThread { ProgressKeeper.submitProgress(ProgressLayout.INSTALL_RESOURCE, progress100, R.string.mod_download_progress, versionItem.fileName) }
    }
}